...
const pet_model_1 = require("../app/modules/pet/pet.model");
...
const ensureStaffUser = async () => {
    const existingStaff = await user_model_1.User.findOne({ role: { $in: ["Admin", "Staff"] } });
    if (existingStaff) {
...
const seedPets = async () => {
    try {
        ...
        for (const pet of samplePets) {
            const existingPet = await pet_model_1.Pet.findOne({ intakeId: pet.intakeId });
            if (existingPet) {
                await pet_model_1.Pet.updateOne({ intakeId: pet.intakeId }, {
                    $set: {
                        ...pet,
...
void seedPets();
//# sourceMappingURL=seedPets.js.map